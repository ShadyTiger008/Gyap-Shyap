...
const UserProfile = ({ toggleUserModal, setToggleUserModal, user }) => {
  console.log(user);
  return (
    <main className="w-screen h-screen bg-[#161719]/80 absolute flex  justify-center items-center text-white">
      <div className="w-[25%] h-fit p-5 rounded-xl flex flex-col justify-center items-center bg-[#202329] space-y-10">
        <div className="w-full flex flex-row ">
          <span className="grow text-center text-3xl font-semibold">{user.name}</span>
          <button onClick={() => setToggleUserModal(false)}>
            <RxCross2 />
          </button>
        </div>
        <img
